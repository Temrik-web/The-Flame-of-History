...
$gameScenePath = 'Assets/All Project/Scenes/Game.unity'
$scene = [IO.File]::ReadAllText((Join-Path $root $gameScenePath))
$blocks = @{}
...
foreach ($override in $overrides) {
    if ($override.Groups[1].Value -eq 'team' -and $override.Groups[2].Value -ne '1') {
        throw "Wrong character team override: $override"
    }
    if ($override.Groups[1].Value -eq 'enemyTeam' -and $override.Groups[2].Value -notin @('0', '1')) {
        throw "Invalid enemy team override: $override"
    }
}
...
if ($buildSettings -notmatch 'enabled: 1\r?\n    path: Assets/All Project/Scenes/Intro\.unity\r?\n    guid: f2c0c58942b22814b8279b8719737309') { throw 'Intro scene is not configured' }
if ($buildSettings -notmatch 'enabled: 1\r?\n    path: Assets/All Project/Scenes/Main menu\.unity\r?\n    guid: 3cfc6fd4d897d744989ff0a5801564fe') { throw 'Main menu scene is not configured' }
if ($buildSettings -notmatch 'enabled: 1\r?\n    path: Assets/All Project/Scenes/Game\.unity\r?\n    guid: 99c9720ab356a0642a771bea13969a05') { throw 'Game scene is not configured' }
